-- NextChapter: Safe Schema Migration Script
-- This script handles existing tables and applies only what's missing

-- Enable required extensions (safe to run multiple times)
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- Drop existing tables if you want a fresh start (UNCOMMENT ONLY IF YOU WANT TO DELETE ALL DATA)
...
-- Layoff details
CREATE TABLE IF NOT EXISTS public.layoff_details (
...
CREATE TABLE IF NOT EXISTS public.bounce_plan_tasks (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
...
-- Apply same pattern for all other tables
DROP POLICY IF EXISTS "Users can manage own layoff details" ON public.layoff_details;
...
DROP POLICY IF EXISTS "Users can manage own budget" ON public.budget_entries;
CREATE POLICY "Users can manage own budget" ON public.budget_entries FOR ALL USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can manage own budget data" ON public.budget_data;
...
DROP POLICY IF EXISTS "Users can manage own bounce plan" ON public.bounce_plan_tasks;
CREATE POLICY "Users can manage own bounce plan" ON public.bounce_plan_tasks FOR ALL USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can manage own coach conversations" ON public.coach_conversations;
CREATE POLICY "Users can manage own coach conversations" ON public.coach_conversations FOR ALL USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can manage own wellness activities" ON public.wellness_activities;
